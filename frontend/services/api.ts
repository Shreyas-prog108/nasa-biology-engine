...
// Use proxy so HTTP-only cookie is sent and token is added by server
...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.code === 'ECONNABORTED') {
      ...
      console.warn('Service unavailable:', error.response.data?.detail)
      const customError = new Error(error.response.data?.detail || 'Service temporarily unavailable')
